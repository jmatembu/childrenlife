	...
	var demoInstall = {
		...
		init: function () {
			$(this.wrapId).on('click', '[data-install]', function (e) {
				e.preventDefault();
				var installationlegit = plugins.installationlegit;
				if(installationlegit === 'false'){
					e.stopImmediatePropagation();
					var setting_page = plugins.setting_page;
		  		if (confirm(plugins.messages.installationlegit_msg)) {
					...
					if (!demoInstall.$link.data('stop-propagation') && plugins['demo_plugins'][demoId] !== undefined) {
						...
						demoInstall.install_plugins(demoId);
					}
				}
			});
		},
		install_plugins: function (demoId) {
			...
			if (plugins['demo_plugins'][demoId] !== undefined) {
				...
				fw.soleModal.show('fw-demo-content-install-plugins',
					...
					'&nbsp;' + plugins.messages.installing + '&nbsp;</h2><p class="fw-text-muted"><em>' + plugins.messages.start_install_plugins + '</em></p>', {
				...
				var data = {
					action: plugins['steps']['install-demo-plugins']['ajax_action'],
					_ajax_nonce: plugins['steps']['install-demo-plugins']['nonce'],
				...
				$.ajax({
					...
					$.get(plugins.admin_url + '?fake=true', function () {
						var success_message = '<h2 class="fw-text-muted">' +
							demoInstall.$loadingImage +
							'&nbsp;' + plugins.messages.installing + '&nbsp;</h2><p class="fw-text-muted"><em>' + plugins.messages.finish_install_plugins + '</em></p>';
