 ...
 */
define("ITEM_ID","15019939"); // Replace it by current item id
define("TOKEN_KEY","BElz0yj5RELlrhl2W4TOO1wPsscHnJXo"); // Replace it by current item id
// Bearsthemes API
class BearsthemesAPI {
    ...
    function request($endpoint, $headers=array(), $curlParams=array()) {
        ...
        try {
            ...
            if ($curl_errno > 0) {
                $message = sprintf("%s (%s): %s \n", __('cURL Error', 'alone'), $curl_errno, $curl_error);
                return json_encode(array( 'error' => $message ) );
            }
        } catch (Exception $e) {
            return null;
...
class VerifyTheme {
    ...
    // add style admin
      ...
      wp_register_style( 'verifytheme', get_stylesheet_directory_uri() . '/theme-includes/includes/verifytheme.css', false );
